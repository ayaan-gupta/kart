...
Written from 2026-09-06, when the owner asked whether a fine-tuned open-weight Qwen would do
...
| [Retrieval review](2026-09-07-retrieval-review.md) | Is the plan built, and is the retrieval this project does the retrieval the problem deserves? | Half built, and the half the plan called most important, logging and corrections, is the unbuilt half. The catalog matcher is real and dark: no enumerator is configured, so every CLUT number is a reader with no catalog. The architecture retrieves on one leg, pictures against pictures, and never uses the text the reader already lifts off the packaging, which is what every published product-linking system does and what the remaining variant-confusion errors need. Ranked next steps at the end. |
