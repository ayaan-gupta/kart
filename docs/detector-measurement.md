...
- Same method is not the same number. Sharpness is measured over a fixed 3x3 grid of 128x128
  tiles of native pixels, so the tiles cover a narrower slice of a 4000px-wide photograph than of
  a 1080p camera frame, and the pixel-scale detail in a photo saved by a phone's camera app is
  not the detail in a preview frame. The bench now gives you the right units and the right ordering.
